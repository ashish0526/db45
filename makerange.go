...
// columns, in order.
func isPKeyPrefix(schema *Schema, cols []string) bool {
...
func destructureCmp(left, right interface{}) (cols []string, vals []*Cell, ok bool) {
	switch l := left.(type) {
	case string:
		r, okr := right.(*Cell)
		if !okr {
...
func typedPrefix(schema *Schema, vals []*Cell) []Cell {
...
func cmpToBound(schema *Schema, cond interface{}) (op ExprOp, cells []Cell, isStart, ok bool) {
