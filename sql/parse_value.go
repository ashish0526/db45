...
// parseInt parses an optional +/- sign followed by digits into a table.TypeI64 cell.
func (p *Parser) parseInt(out *table.Cell) error {
	...
	*out = table.Cell{Type: table.TypeI64, I64: n}
	return nil
}

// parseString parses a quoted string into a table.TypeStr cell. It opens with ' or "
...
func (p *Parser) parseString(out *table.Cell) error {
	...
	for p.pos < len(p.buf) {
		...
		case ch == quote:
			p.pos++
			*out = table.Cell{Type: table.TypeStr, Str: sb}
			return nil
		default:
			sb = append(sb, ch)
			p.pos++
