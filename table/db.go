...
// over the KV storage engine. The SQL front end and its system catalog live in
// the sql package, one level up.
type DB struct {
	KV storage.KV
}
...
func (db *DB) Insert(schema *Schema, row Row) (updated bool, err error) {
	return db.KV.SetEx(row.EncodeKey(schema), row.EncodeVal(schema), storage.ModeInsert)
}

// Upsert writes a complete row, overwriting any existing one.
func (db *DB) Upsert(schema *Schema, row Row) (updated bool, err error) {
	return db.KV.SetEx(row.EncodeKey(schema), row.EncodeVal(schema), storage.ModeUpsert)
}
...
func (db *DB) Update(schema *Schema, row Row) (updated bool, err error) {
	return db.KV.SetEx(row.EncodeKey(schema), row.EncodeVal(schema), storage.ModeUpdate)
}
